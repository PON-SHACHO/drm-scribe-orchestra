...
function getSystemPrompt(contentType: string): string {
  ...
  switch (contentType) {
    case 'free_content':
      return `あなたは心理学とマーケティングに精通した最高レベルのライターです。日本語で回答してください。`;
    case 'sales_letter':
      return `あなたは神田昌典やゲイリーハルバートも驚くセールスレターの専門家です。「12ステップ構成 × 構文活用 × 教育・興味付け・ラベリング挿入」を完全に理解し、読者の感情と行動を同時に動かす長文セールスレターを作成できる、プロフェッショナルです。依頼者は、情報商材を販売する情報商材屋さんです。日本語で回答してください。`;
    case 'short_lp':
...
無料プレゼントコンテンツを執筆してください。（約5000文字）

この無料コンテンツは、【商品情報】の商品を販売するためのリードマグネットの役割を果たします。

商品情報の要約や派生ではなく、価値提供をメインとしつつ商品を購入したくなる動機付けや興味関心を駆り立てるものにしてください。

読者の「認知・感情・行動の変化ステップ」に沿って構造化してください。

以下のガイドラインに従って設計・執筆を行います：

- **章立て構成（3〜5章）**を用い、読者の「気づき→理解→行動」のステージに沿って展開

各章には、PREP構文・逆説構文・SDS構文などの論理構成を戦略的に割り当て、読者の納得感・共感・実行意欲を高める

単なる情報提供ではなく、読者に「そうか、私もやってみよう」と思わせる体験的・内省的な読み物として執筆する

ナレッジ参照：構文マッピング表、および読者変化導線に基づく構成設計ガイド
